...
Вызовите библиотеку из Python с новым токеном для Алгопака, а также с логином/паролем от сайта МосБиржи для подписок:

```python
from MOEXPy import MOEXPy

mp_provider = MOEXPy('<Токен>', '<Логин>', '<Пароль>') 
```

Данные сохранятся в защищенном системном хранилище. Далее можно вызывать библиотеку без токена: mp_provider = MOEXPy()
