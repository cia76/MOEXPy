...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    datanames = ('TQBR.SBER', 'TQBR.HYDR', f'RFUD.{get_future_on_date("Si")}', f'RFUD.{get_future_on_date("RI")}', 'RFUD.CNYRUBF', 'RFUD.IMOEXF')  # Кортеж тикеров
    ...
    for dataname in datanames:  # Пробегаемся по всем тикерам
        ...
        market, marketplace, engine = mp_provider.get_market_engine(board)  # Рынок и торговая площадка
        logger.info(f'Информация о тикере {data_securities[col_securities["BOARDID"]]}.{data_securities[col_securities["SECID"]]} ({data_securities[col_securities["SHORTNAME"]]}, {market}, {marketplace}, {engine})')
        logger.info(f'- Лот: {data_securities[col_securities["LOTSIZE"]] if market == 'shares' else data_securities[col_securities["LOTVOLUME"]]}')
